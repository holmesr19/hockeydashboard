...
export class HistoricalplayersComponent implements OnInit {
  ...
  constructor(
    private playerService: HistoricalPlayerService,
    public fb: FormBuilder
  ) {}

  ngOnInit() {
    this.showPlayerCards = false;
    this.playerService.getYears().subscribe((data) => (this.years = data));
  }
  ...
  getTeams(year: string) {
    ...
    this.playerService.getTeams(year).subscribe((data) => {
      this.teams = data;
    });
  }
